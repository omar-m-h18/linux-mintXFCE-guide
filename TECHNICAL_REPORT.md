...
- **Guided Focus Subsystem (session-only, approved 2026)**: A "lens, not a lock" tour that walks the visitor one goal at a time. `GUIDE_STAGES` (`1..5`) binds each stage to a `moduleId`, plain-language `goal`, `what`/`why`/`help` text, and ordered `steps` (stable selector + hint). `guideState` = `{ current, mode:'idle'|'focused', stepIndex, completed }`, entirely in memory and **never** persisted. The Stage Track (`section.gami-strip` → `.stage-track`) renders `▶ Start guided tour`, five `.stage-chip[data-stage]` jump buttons, `#stage-goal-line`, and `#stage-complete`. In focus mode `#coach-layer` (> `#coach-hole` spotlight + `#coach-bubble` dialog) highlights the current control while other stations are dimmed via `.is-guide-dimmed` — visually only, never `disabled`/`inert`. Completion rides the existing badge path: `gamiEarnBadge(moduleId)` → `guideCompleteStage(moduleId)` advances the track and recaps. The **📖 Guide** drawer (`#guide-drawer`) reuses `trapTab`/`restoreFocus`; Esc exits focus or closes the drawer; `guideScrollToEl` and the coach pop honor `prefers-reduced-motion`.
...
- [x] Start the guided tour and confirm the spotlight targets stage 1, each completed goal advances the Stage Track, and **Free explore** / Esc exits without locking anything.
- [x] Confirm guided state writes **zero** new `safeStorage` keys and that all five stations remain interactive while focus mode is active.
